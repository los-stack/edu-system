        ...
        return (
            <div className="max-w-2xl mx-auto mt-20 bg-white p-10 rounded-3xl shadow-lg border border-gray-100 text-center">
                ...
                <div className="text-6xl font-black text-blue-600 mb-8">
                    {resultScore} <span className="text-2xl text-gray-400">/ 100</span>
                </div>
                
                <button onClick={() => navigate(`/course/${quiz.course_id}`)} className="px-8 py-3 bg-gray-900 text-white font-bold rounded-xl hover:bg-gray-800 transition-colors">
        ...
        <div className="max-w-3xl mx-auto pb-20 pt-10 px-4">
                        ...
                        <div className="space-y-3 pl-2 sm:pl-6">
                            {q.options.map(opt => (
                                <label key={opt.id} className={`flex items-center gap-4 p-4 rounded-xl border cursor-pointer transition-colors ${answers[q.id] === opt.id ? 'bg-blue-50 border-blue-400' : 'bg-white border-gray-200 hover:border-blue-200 hover:bg-gray-50'}`}>
                                    ...
                                    />
                                    <span className={`text-sm sm:text-base ${answers[q.id] === opt.id ? 'font-medium text-blue-900' : 'text-gray-700'}`}>
                                        {opt.answer_text}
                                    </span>
                                </label>
                            ))}
            ...
            <div className="mt-10 flex justify-end">
                <button 
                    ...
                    className="px-8 py-4 bg-blue-600 text-white text-lg font-bold rounded-xl hover:bg-blue-700 transition-colors shadow-lg shadow-blue-600/30 disabled:opacity-50 flex items-center gap-2"
                >
