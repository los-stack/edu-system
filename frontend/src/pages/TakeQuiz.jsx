...
function TakeQuiz() {
    ...
    if (!quiz) return <div className="flex justify-center items-center h-[60vh]"><div className="animate-spin rounded-full h-12 w-12 border-b-4 border-blue-600"></div></div>;
    ...
    if (resultScore !== null) {
        ...
        return (
            <div className="min-h-[70vh] flex items-center justify-center p-4">
                <div className="max-w-xl w-full bg-white dark:bg-[#0f172a] p-10 sm:p-14 rounded-[2.5rem] shadow-2xl border border-gray-100 dark:border-gray-800/80 text-center relative overflow-hidden">
                    ...
                    <div className="relative z-10">
                        ...
                        <p className="text-gray-500 dark:text-gray-400 font-medium mb-8">Ваш результат автоматично збережено в системі.</p>
                        ...
                        <button onClick={() => navigate(`/course/${quiz.course_id}`)} className="w-full py-4 bg-gray-900 dark:bg-white text-white dark:text-gray-900 font-black rounded-xl hover:scale-[1.02] transition-transform shadow-lg">
    ...
    return (
        <div className="max-w-4xl mx-auto pb-32 pt-4 px-4 sm:px-6 relative">
            ...
            <div className="sticky top-16 z-30 bg-gray-50/90 dark:bg-gray-950/90 backdrop-blur-xl pt-4 pb-6 border-b border-transparent">
                <div className="flex justify-between items-end mb-3">
                    <div>
                        <h1 className="text-xl sm:text-2xl font-black text-gray-900 dark:text-white tracking-tight truncate max-w-50 sm:max-w-md">{quiz.title}</h1>
                        <p className="text-sm font-bold text-gray-500 dark:text-gray-400">Питання {answeredCount} з {totalQuestions}</p>
                    </div>
            ...
            <div className="mt-8 mb-12">
                <p className="text-gray-600 dark:text-gray-400 font-medium text-lg leading-relaxed">{quiz.description}</p>
            </div>

            <div className="space-y-10 sm:space-y-16">
                {quiz.questions.map((q, index) => (
                    <div key={q.id} className="relative bg-white dark:bg-[#0f172a] p-6 sm:p-10 rounded-4xl border border-gray-100 dark:border-gray-800/80 shadow-sm transition-colors">
                        
                        <div className="absolute -top-4 -left-2 sm:-left-6 w-12 h-12 bg-blue-600 text-white rounded-2xl flex items-center justify-center font-black text-xl shadow-lg shadow-blue-600/30 transform -rotate-6">
                            {index + 1}
                        </div>

                        <h3 className="text-xl sm:text-2xl font-extrabold text-gray-900 dark:text-white leading-snug mb-8 mt-2 pl-4 sm:pl-6">
                            {q.question_text}
                        </h3>
                            ...
                            {q.options.map(opt => {
                                ...
                                return (
                                    <div 
                                        ...
                                        className={`group flex items-center gap-4 p-5 rounded-2xl border-2 cursor-pointer transition-all duration-200 ${isSelected ? 'border-blue-600 bg-blue-50 dark:bg-blue-500/10' : 'border-gray-200 dark:border-gray-700/50 hover:border-blue-300 dark:hover:border-gray-500 hover:bg-gray-50 dark:hover:bg-gray-800/50'}`}
                                    >
                                        <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center shrink-0 transition-colors ${isSelected ? 'border-blue-600 dark:border-blue-400' : 'border-gray-300 dark:border-gray-600 group-hover:border-blue-400'}`}>
                                        ...
                                        <span className={`text-base sm:text-lg font-medium ${isSelected ? 'text-blue-900 dark:text-blue-300 font-bold' : 'text-gray-700 dark:text-gray-300'}`}>
                                            {opt.answer_text}
                                        </span>
                                    </div>
                                );
            ...
            <div className="fixed bottom-0 left-0 right-0 p-4 sm:p-6 bg-linear-to-t from-gray-50 via-gray-50/90 to-transparent dark:from-gray-950 dark:via-gray-950/90 pointer-events-none flex justify-center z-40">
                <button 
                    onClick={handleAttemptSubmit} 
                    disabled={isSubmitting || progressPercentage < 100} 
                    className="pointer-events-auto px-10 py-4 sm:py-5 bg-blue-600 text-white text-base sm:text-lg font-black tracking-wide rounded-2xl hover:bg-blue-500 hover:-translate-y-1 transition-all shadow-xl shadow-blue-600/30 disabled:opacity-50 disabled:hover:translate-y-0 disabled:shadow-none flex items-center gap-3"
                >
                    {isSubmitting ? (
                        <>
