...
function CoursePage() {
    ...
    const handleStudentSubmit = async (e, assignmentId) => {
        ...
        try {
            ...
            const promise = axios.post(`/api/assignments/${assignmentId}/submit`, formData, {
                headers: { 'Content-Type': 'multipart/form-data' }
            });
            ...
            <div className="relative bg-white dark:bg-zinc-900 rounded-[2.5rem] p-8 sm:p-12 mb-10 border border-zinc-200 dark:border-zinc-800 shadow-xl shadow-zinc-200/40 dark:shadow-none overflow-hidden">
                ...
                <div className="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-end gap-8">
                    <div className="max-w-2xl w-full">
                        ...
                        <div className="prose prose-lg dark:prose-invert text-zinc-500 dark:text-zinc-400 max-w-none line-clamp-2 wrap-break-word overflow-hidden" dangerouslySetInnerHTML={{ __html: course.description }}></div>
                    </div>
                                                    ...
                                                    <div className="mt-6 p-6 bg-zinc-50 dark:bg-zinc-800/40 rounded-3xl border border-zinc-100 dark:border-zinc-700/50">
                                                        <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
                                                            <div className="flex items-center gap-3">
                                                                <div className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 ${mySub ? 'bg-green-100 text-green-600 dark:bg-green-500/20 dark:text-green-400' : 'bg-zinc-200 text-zinc-400 dark:bg-zinc-700 dark:text-zinc-500'}`}>
                                                            ...
                                                            <form onSubmit={(e) => handleStudentSubmit(e, assignment.id)} className="flex flex-col sm:flex-row gap-2 w-full sm:w-auto mt-4 sm:mt-0">
                                                                <input 
                                                                    type="file" 
                                                                    id={`studentFile-${assignment.id}`} 
                                                                    className="hidden" 
                                                                    onChange={(e) => {
                                                                        if(e.target.files[0]) {
                                                                            setSelectedFiles(prev => ({...prev, [assignment.id]: e.target.files[0]}));
                                                                        }
                                                                    }} 
                                                                />
                                                                
                                                                <label 
                                                                    htmlFor={`studentFile-${assignment.id}`} 
                                                                    className={`cursor-pointer px-4 py-2 border text-xs font-bold rounded-xl transition-all truncate max-w-50 text-center ${
                                                                        selectedFile 
                                                                        ? 'bg-blue-50 border-blue-200 text-blue-700 dark:bg-blue-500/10 dark:border-blue-500/30 dark:text-blue-400' 
                                                                        : 'bg-white dark:bg-zinc-700 border-zinc-200 dark:border-zinc-600 hover:bg-zinc-100 dark:hover:bg-zinc-600 text-zinc-700 dark:text-zinc-300'
                                                                    }`}
                                                                    title={selectedFile ? selectedFile.name : 'Обрати файл'}
                                                                >
                                                                ...
                                                                <button 
                                                                    ...
                                                                    className="px-4 py-2 bg-blue-600 text-white text-xs font-black rounded-xl hover:bg-blue-500 shadow-md shadow-blue-500/20 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                                                                >
                                                                    ВІДПРАВИТИ
                                                                </button>
                                                            </form>
                                    ...
                                    <div key={`${sub.assignment_id}-${sub.student_id}`} className="p-6 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-3xl flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6 shadow-xl shadow-zinc-200/40 dark:shadow-none">
                                        <div className="flex items-center gap-4">
                                            ...
                                            <div>
                                                ...
                                                <a href={sub.file_url} target="_blank" rel="noreferrer" className="text-xs font-bold text-blue-500 hover:text-blue-400 transition-colors">ЗАВАНТАЖИТИ ФАЙЛ</a>
                                            </div>
                                        </div>
                                        {sub.score !== null ? (
                                            ...
                                            <form onSubmit={(e) => handleGradeSubmit(e, sub.assignment_id, sub.student_id)} className="flex flex-col sm:flex-row items-center gap-2 w-full lg:w-auto">
                                                <input type="number" name="scoreInput" placeholder="Бал" min="0" max="100" className="w-full sm:w-20 px-3 py-2 bg-zinc-50 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-xl text-sm font-bold text-zinc-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500" required />
                                                <input type="text" name="feedbackInput" placeholder="Відгук..." className="w-full flex-1 lg:w-64 px-4 py-2 bg-zinc-50 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-xl text-sm text-zinc-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500" />
                                                <button type="submit" className="w-full sm:w-auto px-6 py-2 bg-zinc-900 dark:bg-white text-white dark:text-zinc-900 text-xs font-black rounded-xl hover:opacity-80 transition-all shadow-sm">ОЦІНИТИ</button>
                                            </form>
                                        )}
                                    </div>
                                ))}
                                ...
                                courseStudents.map(s => (
                                    <div key={s.id} className="flex items-center gap-4 p-4 rounded-2xl hover:bg-zinc-50 dark:hover:bg-zinc-800/50 transition-colors border border-transparent hover:border-zinc-100 dark:hover:border-zinc-700/50">
                                        ...
                                        <div className="overflow-hidden">
                                            <span className="block font-bold text-zinc-900 dark:text-zinc-200 truncate">{s.name}</span>
                                            {(user.role === 'teacher' || user.role === 'admin') && <span className="text-xs text-zinc-500 truncate block">{s.email}</span>}
                                        </div>
                                    </div>
                                ))
                            ) : (
